...
class ConnectionDelayMetricsExtensionTester(AsyncExtensionTester):
    ...
    async def on_data(
        ...
        if data_name == "metrics":
            ...
            if not self.stop_test_if_checking_failed(
                ten_env_tester,
                vendor_metadata.get("region") == "fake_region",
                f"unexpected vendor region: {vendor_metadata}",
            ):
                return
            if not self.stop_test_if_checking_failed(
                ten_env_tester,
                vendor_metadata.get("key") == encrypt("fake_key"),
                f"unexpected encrypted key: {vendor_metadata.get('key')}",
            ):
                return
...
def test_connection_delay_metrics(patch_azure_ws):
    ...
    5. metadata includes vendor_metadata with region, language, and encrypted key
    """
