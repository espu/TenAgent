...
class TestNvidiaRivaAPICompliance:
    ...
    def test_auth_initialization_parameters(self, valid_config, mock_ten_env):
        ...
        with patch('nvidia_riva_tts_python.riva_tts.riva.client.Auth') as mock_auth, \
    ...
    def test_speech_synthesis_service_initialization(self, valid_config, mock_ten_env):
        """Verify SpeechSynthesisService is initialized with Auth object"""
        with patch('nvidia_riva_tts_python.riva_tts.riva.client.Auth') as mock_auth, \
             patch('nvidia_riva_tts_python.riva_tts.riva.client.SpeechSynthesisService') as mock_service:
    ...
    @pytest.mark.asyncio
    async def test_synthesize_online_parameters(self, valid_config, mock_ten_env):
        """Verify synthesize_online is called with correct parameters per Riva API"""
        with patch('nvidia_riva_tts_python.riva_tts.riva.client.Auth'), \
             patch('nvidia_riva_tts_python.riva_tts.riva.client.SpeechSynthesisService') as mock_service, \
             patch('nvidia_riva_tts_python.riva_tts.riva.client.AudioEncoding') as mock_encoding:
            ...
            # Synthesize text
            text = "Hello world"
            chunks = [chunk async for chunk in client.synthesize(text, "test_request")]
    ...
    async def test_audio_encoding_linear_pcm(self, valid_config, mock_ten_env):
        ...
        with patch('nvidia_riva_tts_python.riva_tts.riva.client.Auth'), \
            ...
            # Synthesize
            chunks = [chunk async for chunk in client.synthesize("Test", "req1")]
    ...
    async def test_audio_format_int16(self, valid_config, mock_ten_env):
        ...
        with patch('nvidia_riva_tts_python.riva_tts.riva.client.Auth'), \
            ...
            mock_audio = np.array([100, -100, 200, -200], dtype=np.int16).tobytes()
            mock_response = Mock()
    ...
    @pytest.mark.asyncio
    async def test_streaming_response_iteration(self, valid_config, mock_ten_env):
        ...
        with patch('nvidia_riva_tts_python.riva_tts.riva.client.Auth'), \
            ...
            for i in range(3):
                mock_response = Mock()
                mock_response.audio = np.array([i] * 10, dtype=np.int16).tobytes()
                mock_responses.append(mock_response)
            
            mock_service_instance = Mock()
            mock_service_instance.synthesize_online = Mock(return_value=mock_responses)
            mock_service.return_value = mock_service_instance
    ...
    def test_required_config_parameters(self):
        ...
        with pytest.raises(ValueError, match="Server address is required"):
            config = NvidiaRivaTTSConfig(
                params={"language_code": "en-US", "voice_name": "English-US.Female-1"}
            )
            config.validate()
        ...
        with pytest.raises(ValueError, match="Language code is required"):
            config = NvidiaRivaTTSConfig(
                params={"server": "localhost:50051", "voice_name": "English-US.Female-1"}
            )
            config.validate()
    ...
    def test_ssl_configuration(self, mock_ten_env):
        ...
        with patch('nvidia_riva_tts_python.riva_tts.riva.client.Auth') as mock_auth, \
            ...
            client = NvidiaRivaTTSClient(config=config_with_ssl, ten_env=mock_ten_env)
    ...
    async def test_cancellation_support(self, valid_config, mock_ten_env):
        ...
        with patch('nvidia_riva_tts_python.riva_tts.riva.client.Auth'), \
            ...
            chunks = []
            async for i, chunk in enumerate(client.synthesize("Long text", "req1")):
                chunks.append(chunk)
