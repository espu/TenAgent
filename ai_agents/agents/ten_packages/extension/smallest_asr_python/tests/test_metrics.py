...
class SmallestAsrExtensionTester(AsyncExtensionTester):
    ...
    async def on_data(
        ...
        if data_name == "asr_result":
            ...
            self.stop_test_if_checking_failed(
                ten_env_tester,
                "vendor" in data_dict and data_dict["vendor"] == "smallest",
...
def test_metrics(patch_smallest_ws):
    def trigger_transcript_messages():
        """Add WebSocket messages to simulate Smallest AI Pulse API responses."""
        # Send a transcript message in the Smallest AI Pulse format
        transcript_message = {
            "type": "transcription",
