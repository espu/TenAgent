...
def _sse_audio(pcm: bytes) -> bytes:
    """Wrap PCM16 bytes in the SSE frame Lightning's live endpoint emits."""
    payload = {"audio": base64.b64encode(pcm).decode()}
    return f"data: {json.dumps(payload)}\n\n".encode()


_SSE_DONE = b'data: {"done": true}\n\n'
...
def test_reconnect_after_connection_drop(
    ...
    def create_mock_response(*args, **kwargs):
        ...
        if stream_call_count == 1:
            ...
            # Mock aiter_bytes() for streaming audio data (SSE frames
            # carrying base64-encoded PCM16, as Lightning's live endpoint
            # returns).
            async def mock_aiter_bytes():
                yield _sse_audio(struct.pack("<2h", 1000, -1000))
                yield _SSE_DONE
    ...
    tester.set_test_mode_single("smallest_tts_python", json.dumps(config))
    ...
    assert (
        vendor_info.get("vendor") == "smallest"
    ), f"Expected vendor 'smallest', got {vendor_info.get('vendor')}"
