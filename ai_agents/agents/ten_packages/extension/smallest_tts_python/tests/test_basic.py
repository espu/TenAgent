...
def test_dump_functionality(MockAsyncClient):
    ...
    # --- Mock Configuration ---
    # Create some fake PCM16 audio and wrap it in the SSE frames Smallest AI's
    # Lightning live endpoint emits (base64-encoded PCM16 `data:` payloads).
    fake_pcm_chunk_1 = struct.pack("<20h", *([1000] * 20))
    fake_pcm_chunk_2 = struct.pack("<20h", *([-2000] * 20))
    ...
    async def mock_aiter_bytes():
        yield _sse_audio(fake_pcm_chunk_1)
        await asyncio.sleep(0.01)
        yield _sse_audio(fake_pcm_chunk_2)
        await asyncio.sleep(0.01)
        yield _SSE_DONE
    ...
    tester.set_test_mode_single("smallest_tts_python", json.dumps(dump_config))
...
class ExtensionTesterFlush(ExtensionTester):
    def __init__(self):
        ...
        self.sample_rate = 24000  # Lightning TTS default sample rate
...
def test_flush_logic(MockAsyncClient):
    ...
    # SSE frame carrying base64-encoded PCM16 audio (100 samples).
    flush_pcm_chunk = struct.pack("<100h", *([1000] * 100))
    ...
    async def mock_aiter_bytes():
        ...
        for i in range(20):
            ...
            if cancel_called:
                ...
                break
            yield _sse_audio(flush_pcm_chunk)
