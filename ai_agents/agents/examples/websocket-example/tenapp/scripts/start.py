...
def load_env_file(app_dir):
    ...
    if env_file.exists():
        print(f"[start.py] Loading environment from {env_file}", file=sys.stderr)
        try:
                ...
                for line in f:
                    ...
                    if line and not line.startswith('#'):
                        if '=' in line:
                            ...
                            print(f"[start.py] Set {key.strip()}", file=sys.stderr)
...
def main():
    ...
    pythonpath_parts = [
        ...
        str(app_dir / "ten_packages" / "system" / "ten_runtime_python" / "interface"),
    ...
    print(f"[start.py] PYTHONPATH set to: {os.environ['PYTHONPATH']}", file=sys.stderr)

    # Set library paths based on platform
    lib_paths = []
    lib_candidates = [
        app_dir / "ten_packages" / "system" / "ten_runtime" / "lib",  # Critical: ten_runtime.dll and ten_utils.dll
    ...
    for lib_path in lib_candidates:
        if lib_path.exists():
            ...
            print(f"[start.py] Adding library path: {lib_path}", file=sys.stderr)
        else:
            print(f"[start.py] Library path not found (skipping): {lib_path}", file=sys.stderr)
    ...
    if platform.system() == "Windows":
        ...
        if lib_paths:
            ...
            print(f"[start.py] Updated PATH with library directories", file=sys.stderr)
    else:
        ...
        if lib_paths:
            ...
            print(f"[start.py] Updated LD_LIBRARY_PATH with library directories", file=sys.stderr)
    ...
    print(f"[start.py] PYTHONPATH: {os.environ.get('PYTHONPATH', 'NOT SET')}", file=sys.stderr)
    print(f"[start.py] Executing: {main_path} {' '.join(sys.argv[1:])}", file=sys.stderr)
    sys.stderr.flush()
